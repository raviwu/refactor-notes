require_relative File.basename(File.expand_path(__FILE__), '.rb').gsub('_test', '')
require 'minitest/autorun'

class TestRental < Minitest::Test
  def setup
    @movie_paul = Movie.new(title: 'Paul', price: NewReleasePrice.new)
    @movie_dory = Movie.new(title: 'Dory', price: ChildrenPrice.new)
    @movie_seven = Movie.new(title: 'se7en', price: RegularPrice.new)
  end
  ...
  def test_statement_new_release
    ...
    expected_statement =
      header(customer.name) +
      line_item(@movie_paul.title => 6.0) +
      footer(total: 6.0, renter_point: 2)
  ...
  def test_multi_rentals
    ...
    expected_statement =
      header(customer.name) +
      line_item(@movie_dory.title => 1.5, @movie_seven.title => 2.0) +
  ...
  def test_html_statement_regular_rental_within_2_days
    ...
    expected_statement =
      html_header(customer.name) +
      html_line_item(@movie_seven.title => 2.0) +
      html_footer(total: 2.0, renter_point: 1)
  ...
  def test_html_statement_new_release
    ...
    expected_statement =
      html_header(customer.name) +
      html_line_item(@movie_paul.title => 6.0) +
      html_footer(total: 6.0, renter_point: 2)
  ...
  def test_html_multi_rentals
    ...
    expected_statement =
      html_header(customer.name) +
      html_line_item(@movie_dory.title => 1.5, @movie_seven.title => 2.0) +
